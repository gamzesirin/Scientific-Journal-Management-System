...
export default async function DashboardPage() {
	...
	switch (role) {
		...
		case 'reviewer':
			...
			const { data: reviewerReviews } = await supabase.from('reviews').select('*').eq('reviewer_id', user.id)
		...
		case 'admin':
			...
			const roleCounts = usersByRole?.reduce(
				(acc: RoleCounts, user: { role: string }) => {
			...
			const { data: papersByStatus } = await supabase.from('articles').select('status')
			...
			const statusCounts = papersByStatus?.reduce(
				(acc: StatusCounts, paper: { status: string }) => {
	...
	return (
		<div className="min-h-screen bg-gradient-to-b from-gray-50 to-gray-100">
			<div className="container mx-auto py-8 px-4 max-w-7xl">
				...
				{role === 'author' && (
					<AuthorDashboard user={user} userData={userData} articles={dashboardData.articles || []} />
				)}

				{role === 'editor' && (
					<EditorDashboard user={user} userData={userData} allPapers={dashboardData.allPapers || []} />
				)}
				...
				{role === 'admin' && dashboardData.stats && (
					<AdminDashboard user={user} userData={userData} stats={dashboardData.stats} />
				)}
			</div>
		</div>
	)
