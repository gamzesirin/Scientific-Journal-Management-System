...
const MOCK_MODE = FORCE_MOCK || (process.env.NODE_ENV === 'development' && !process.env.GEMINI_API_KEY);
...
export const GEMINI_MODEL = 'gemini-1.5-pro-latest'; // Using the latest stable Gemini model
...
function generateMockCVAnalysis(cvText: string, cvUrl: string) {
  ...
  return {
    research_areas: [
      { area: "Computer Science", weight: 0.9 },
      { area: "Machine Learning", weight: 0.8 },
      { area: "Artificial Intelligence", weight: 0.7 }
    ],
    keywords: [
      "machine learning", "deep learning", "neural networks",
      "data science", "computer vision", "NLP", "AI",
      "Python", "TensorFlow", "research"
    ],
    technical_skills: ["Python", "R", "MATLAB", "TensorFlow", "PyTorch", "SQL"],
    methodologies: ["supervised learning", "unsupervised learning", "experimental design", "statistical analysis"],
    domains: ["Computer Science", "Artificial Intelligence", "Data Science"],
    publications_count: 10 + (hash % 20),
    h_index: 5 + (hash % 10),
    years_of_experience: 3 + (hash % 7),
    expertise_score: score,
    cv_analysis_summary: "Mock analysis: This is a simulated CV analysis for testing purposes. The reviewer appears to have expertise in computer science and machine learning.",
...
export async function analyzeCVWithAI(cvText: string, cvUrl: string) {
  ...
  // Use mock mode if enabled or API is not accessible
  if (MOCK_MODE || !API_ACCESSIBLE) {
    console.log('[Gemini] Using MOCK MODE - API not accessible or mock mode enabled');
...
Return ONLY valid JSON, no additional text.`;
    ...
    if (error?.message?.includes('fetch failed') || error?.message?.includes('network')) {
      ...
      console.log('[Gemini] Switching to MOCK mode for this session');
    ...
    if (error?.status === 429 || error?.message?.includes('429') || error?.message?.includes('quota')) {
      ...
      console.log('[Gemini] Falling back to MOCK mode for this request');
    ...
    // For any other errors, use mock mode
    console.error('[Gemini] Unexpected error, using MOCK mode');
...
export async function analyzeArticleWithAI(
  ...
  // Use mock mode if enabled or API is not accessible
  if (MOCK_MODE || !API_ACCESSIBLE) {
    console.log('[Gemini] Using MOCK MODE for article analysis');
    return {
      main_topics: [
      ...
      analysis_summary: "Mock analysis: This article appears to be about machine learning research.",
      gemini_model_version: "mock-model"
    };
  }
...
Return ONLY valid JSON, no additional text.`;
    ...
    // Handle all errors by falling back to mock mode
    if (error?.message?.includes('fetch failed') ||
        error?.message?.includes('network') ||
        error?.status === 429 ||
        error?.status === 401 ||
        error?.status === 404 ||
        true) { // Always fallback for any error
      console.log('[Gemini] Error in article analysis, using mock data');
      API_ACCESSIBLE = false;
      LAST_API_CHECK = Date.now();
      ...
      return {
        ...
        research_domain: "Computer Science",
        complexity_level: "intermediate",
        analysis_summary: "Mock analysis: Article analysis using simulated data.",
        gemini_model_version: "mock-model"
      };
    }
  }
...
export async function calculateMatchingScore(
  ...
  // Use mock mode if enabled or API is not accessible
  if (MOCK_MODE || !API_ACCESSIBLE) {
    console.log('[Gemini] Using MOCK MODE for matching score');
    return {
      ...
      matching_keywords: ["machine learning", "AI"],
      matching_topics: ["Computer Science"],
      mismatch_reasons: [],
...
Return ONLY valid JSON.`;
    ...
    // Always fallback to mock mode for any error
    console.log('[Gemini] Error in matching score calculation, using mock data');
    API_ACCESSIBLE = false;
    LAST_API_CHECK = Date.now();
    ...
    return {
      ...
      recommendation_level: "good",
      confidence_score: 80,
      explanation: "Mock matching: Using simulated scoring due to API issues.",
      gemini_model_version: "mock-model"
    };
  }
}